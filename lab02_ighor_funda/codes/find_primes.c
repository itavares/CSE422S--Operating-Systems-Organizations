...
static void second_barrier(void){
	...
	//spin_unlock_irqrestore(&barrier2_lock, flags2);
...
static void mark_non_primes(int *counters){
	...
	while(!finish){
		...
		if(my_curr_pos >= upper_bound-1){
			...
			curr_num = numbers[my_curr_pos];
			for( i=my_curr_pos+curr_num; i < upper_bound-1; i=i+curr_num){
				mutex_lock(&crossed_out_lock);
				numbers[i] = 0;
				mutex_unlock(&crossed_out_lock);
...
static int simple_init(void)
{
	...
	numbers = (int *)kmalloc((upper_bound-1)*sizeof(int), GFP_KERNEL);
	if(!numbers){
		numbers =0;
		counters=0;
	...
	counters = (int *)kmalloc((num_threads)*sizeof(int), GFP_KERNEL);
	if(!counters){
